<?php 
	...
	$obj=new admin();
    $adv_details=$obj->select_objData("adv_tbl3","order by adv_id");
	$menu_details=$obj->select_objData("menu","");
     if(isset($_POST['ADV_new']))
	{
		$insert_menu=$obj->insert_ADV("adv_tbl3");
...
{
$sl_Q=mysqli_query($con,"select * from adv_tbl3 where adv_id=".$_REQUEST['id']);
$adData=mysqli_fetch_array($sl_Q); 
unlink("product/forad3/".$adData['img']);
	if(mysqli_query($con,"delete from adv_tbl3 where adv_id=".$_REQUEST['id']))
	{	?><script type="text/javascript">
...
if($_REQUEST['action']=='Inactive'){
    if(mysqli_query($con,"update adv_tbl3 set status='Inactive' where adv_id=".$_REQUEST['id'])){
	?>
	<script>
...
if($_REQUEST['action']=='Active'){
    if(mysqli_query($con,"update adv_tbl3 set status='Active' where adv_id=".$_REQUEST['id'])){
	?>
	<script>
            ...
            <div class="col-lg-12">
               ...
               <?php $sel_cat=mysqli_fetch_array(mysqli_query($con,"select category_name from categories where cat_id='$cat_id'"))?>
                ...
                <td class="hidden-phone"><?php echo $sel_subcat['subcat_name'];?></td>
						<td><?php echo $Adv;?></td>
                        <td><img src="product/forad3/<?php echo $img; ?>"  height="50px"/></td>
